...
public class Banner extends BaseController {
	...
	public String movebanner(HttpServletRequest request) {
		...
		logger.info(request.getSession().getAttribute("name")+"移动广告位的管理员 "+request.getSession().getAttribute("name")+"广告位id"+currindex);
		String cpath = request.getContextPath();
		String cbasePath = request.getScheme() + "://"
				+ request.getServerName() + ":" + request.getServerPort()
				+ cpath + "/";
	...
	public String updateBanner(HttpServletRequest request) {
		...
		logger.info(request.getSession().getAttribute("name")+"创建广告位的管理员 "+request.getSession().getAttribute("name")+"广告位name"+name);
		return "redirect:/banner/bannerlist";
	}
