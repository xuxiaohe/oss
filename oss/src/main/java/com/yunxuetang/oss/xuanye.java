...
public class xuanye extends BaseController{
	...
	public ModelAndView bindBoxDry(HttpServletRequest request) {
		...
		logger.info(request.getSession().getAttribute("name")+"炫页关联到具体的排行榜操作的管理员 "+request.getSession().getAttribute("name")+"===位置id"+boxPostId+"===炫页id"+sourceId);
	...
	public String deleteDry(HttpServletRequest request) {
		// 必输
		String dryid = request.getParameter("dryid");
		logger.info(request.getSession().getAttribute("name")+"删除炫页操作的管理员 "+request.getSession().getAttribute("name")+"炫页id"+dryid);
		deleteDryById(dryid);
		return "redirect:/xuanye/xuanyeList";
	}
