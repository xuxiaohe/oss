...
<body>
	...
	<div class="panel-body">
		...
		<div class="panel panel-default">
			...
			<c:forEach items="${addDryBoxposition.data.result}" varStatus="key"
						var="Recourse">
				<li><a href="${cbasePath}course/courseBoxList?id=${Recourse.id}&name=${Recourse.chinaName}">${Recourse.chinaName}</a></li>
				</c:forEach>
		...
		<div class="btn-group">
		<form class="form-inline" action="${cbasePath}course/searchcourseBoxList?id=${id}&name=${name}"
				...
				<c:if test="${addDryBoxList.status == '200'}">
					...
					<ul class="pagination">
						<pageNation:PageNation currPage="${addDryBoxList.data.curr_page}"
							...
							linkBaseUrl="${cbasePath}course/courseBoxList?id=${id}&name=${name}">
