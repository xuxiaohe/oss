...
<body>
	...
	<div class="panel-body">
		...
		<div class="panel panel-default">
			...
			<c:forEach items="${addDryBoxposition.data.result}" varStatus="key"
						var="Recourse">
				<li><a href="${cbasePath}topic/TopicBoxList?id=${Recourse.id}&name=${Recourse.chinaName}">${Recourse.chinaName}</a></li>
				</c:forEach>
		...
		<br><br>
		<div class="btn-group">
		<form class="form-inline" action="${cbasePath}topic/TopicBoxListBySearch?id=${id}&name=${name}"
				...
				<c:if test="${addDryBoxList.status == '200'}">
					...
					<ul class="pagination">
						<pageNation:PageNation currPage="${addDryBoxList.data.curr_page}"
							...
							linkBaseUrl="${cbasePath}topic/TopicBoxList?id=${id}&name=${name}">
					...
					<c:forEach items="${addDryBoxList.data.result}" varStatus="key"
						...
						<div class="row" style="padding: 20px;">
							...
							<div class="col-xs-10">
								...
								<a href="${cbasePath}topic/bindBoxTopic?boxPostId=${id}&sourceId=${Recourse.topicId}&name=${name}&ctime=${Recourse.ctime}">
