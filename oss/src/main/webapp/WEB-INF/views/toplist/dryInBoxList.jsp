	...
	<div class="panel-body">
		<div class="btn-group">
			...
			<c:forEach items="${addDryBoxposition.data.result}" varStatus="key"
						var="Recourse">
				<li><a href="${cbasePath}dry/DryInBoxList?id=${Recourse.id}&name=${Recourse.chinaName}">${Recourse.chinaName}</a></li>
				</c:forEach>
				...
				<c:if test="${addDryBoxList.status == '200'}">
					...
					<ul class="pagination">
						<pageNation:PageNation currPage="${addDryBoxList.data.curr_page}"
							...
							linkBaseUrl="${cbasePath}area/InBoxList?id=${id}&type=${type}&name=${name}">
					...
					<c:forEach items="${addDryBoxList.data.result}" varStatus="key"
						...
						<div class="row" style="padding: 20px;">
							...
							<div class="col-xs-1">
								<div class="row">
									<img class="col-xs-12 thumbnail" src="${Recourse.sourceId.fileUrl}"
										style="margin-top: 10px;" alt="" />
								</div>
							</div>
							<div class="col-xs-10">
								<h4 style="margin-left: 12px;">
									<a href="${cbasePath}dry/dryDetail?dryid=${Recourse.sourceId.id}">
										${Recourse.sourceId.message} </a><br> <small><small
										class="pull-right">注册时间：<Date:date
												value="${Recourse.sourceId.ctime}"></Date:date></small></small>
												<br>权重：${Recourse.sourceId.weightSort}
								</h4>


								<div class="col-xs-12 btn-group-sm">
								<a href="${cbasePath}area/unbindBox?boxPostId=${id}&boxId=${Recourse.id}&name=${name}&type=${type}">
